...
VOID TEST(ProtocolStackTest, ProtocolRtmpPreferCid)
{
    EXPECT_EQ(RTMP_CID_ProtocolControl, srs_rtmp_prefer_cid(RTMP_MSG_SetPeerBandwidth));
    EXPECT_EQ(RTMP_CID_ProtocolControl, srs_rtmp_prefer_cid(RTMP_MSG_SetChunkSize));
    EXPECT_EQ(RTMP_CID_OverConnection, srs_rtmp_prefer_cid(RTMP_MSG_AMF0CommandMessage));
    EXPECT_EQ(RTMP_CID_OverConnection, srs_rtmp_prefer_cid(RTMP_MSG_AMF3CommandMessage));
    EXPECT_EQ(RTMP_CID_OverStream, srs_rtmp_prefer_cid(RTMP_MSG_AMF0DataMessage));
    EXPECT_EQ(RTMP_CID_Video, srs_rtmp_prefer_cid(RTMP_MSG_VideoMessage));
    EXPECT_EQ(RTMP_CID_Audio, srs_rtmp_prefer_cid(RTMP_MSG_AudioMessage));
}

VOID TEST(ProtocolStackTest, ProtocolSendSrsConnectAppResPacketPreferCid)
{
    srs_error_t err = srs_success;
    ...
    HELPER_EXPECT_SUCCESS(proto.send_and_free_packet(pkt, 0));
    EXPECT_GT(bio.out_buffer.length(), 0);
    EXPECT_EQ(RTMP_CID_OverConnection, (uint8_t)bio.out_buffer.bytes()[0]);
}
