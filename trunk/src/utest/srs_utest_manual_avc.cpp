...
VOID TEST(SrsAVCTest, H264IPBFrame)
{
    ...
    if (true) {
        ...
        EXPECT_EQ(0, (uint8_t)frame.at(0));
        EXPECT_EQ(0, (uint8_t)frame.at(1));
        EXPECT_EQ(0, (uint8_t)frame.at(2));
        EXPECT_EQ(5, (uint8_t)frame.at(3));
...
VOID TEST(SrsAVCTest, AACDemuxADTS)
{
    ...
    if (true) {
        ...
        uint8_t b[] = {0xff, 0x09, 0x2c, 0x40, 0, 0xe0, 0};
    ...
    if (true) {
        ...
        int nb_frame = 0;
        SrsRawAacStreamCodec codec;
        uint8_t b[] = {0xff, 0xf0, 0x10, 0x40, 0x04, 0, 0, 1};
        SrsBuffer buf((char *)b, sizeof(b));
        HELPER_EXPECT_FAILED(h.adts_demux(&buf, &frame, &nb_frame, codec));
    }
    ...
    if (true) {
        ...
        int nb_frame = 0;
        SrsRawAacStreamCodec codec;
        uint8_t b[] = {0xff, 0xf0, 0x10, 0x40, 0x01, 0, 0, 1};
        SrsBuffer buf((char *)b, sizeof(b));
        HELPER_EXPECT_FAILED(h.adts_demux(&buf, &frame, &nb_frame, codec));
    }
    ...
    if (true) {
        ...
        int nb_frame = 0;
        SrsRawAacStreamCodec codec;
        uint8_t b[] = {0xff, 0xf1, 0x10, 0x40, 0x01, 0, 0, 1};
        SrsBuffer buf((char *)b, sizeof(b));
        HELPER_ASSERT_SUCCESS(h.adts_demux(&buf, &frame, &nb_frame, codec));
    }
