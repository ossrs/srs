...
static srs_error_t check_remote_sdp(const SrsSdp& remote_sdp, bool is_publish)
{
    ...
    for (std::vector<SrsMediaDesc>::const_iterator iter = remote_sdp.media_descs_.begin(); iter != remote_sdp.media_descs_.end(); ++iter) {
        ...
        for (std::vector<SrsMediaPayloadType>::const_iterator iter_media = iter->payload_types_.begin(); iter_media != iter->payload_types_.end(); ++iter_media) {
            if (is_publish ? iter->recvonly_ : iter->sendonly_) {
                return srs_error_new(ERROR_RTC_SDP_EXCHANGE, "unsupported media stream direction");
            }
        }
    }
...
static srs_error_t do_serve_rtc_request(ISrsHttpResponseWriter* w, ISrsHttpMessage* r, SrsJsonObject* res, SrsRtcServer* server, bool is_publish)
{
    srs_error_t err = srs_success;
    ...
    SrsJsonObject* reqJsonObj = NULL;
    SrsAutoFree(SrsJsonObject, reqJsonObj);
    if (true) {
        string req_json;
        ...
        reqJsonObj = json->to_object();
    }

    // Fetch params from req object.
    SrsJsonAny* prop = NULL;
    if ((prop = reqJsonObj->ensure_property_string("sdp")) == NULL) {
        return srs_error_wrap(err, "not sdp");
    }
    string remote_sdp_str = prop->to_str();

    if ((prop = reqJsonObj->ensure_property_string("streamurl")) == NULL) {
        return srs_error_wrap(err, "not streamurl");
    }
    string streamurl = prop->to_str();

    string clientip;
    if ((prop = reqJsonObj->ensure_property_string("clientip")) != NULL) {
    ...
    string api;
    if ((prop = reqJsonObj->ensure_property_string("api")) != NULL) {
        api = prop->to_str();
    }

    string tid;
    if ((prop = reqJsonObj->ensure_property_string("tid")) != NULL) {
        tid = prop->to_str();
    }

    string stream_name, tcUrl;
    ...
    srs_trace("RTC signaling request %s, api=%s, tid=%s, clientip=%s, app=%s, stream=%s, offer=%dB, eip=%s, codec=%s, srtp=%s, dtls=%s",
    ...
    if ((err = check_remote_sdp(ruc.remote_sdp_, is_publish)) != srs_success) {
    ...
    if ((err = server->create_session(&ruc, local_sdp, &session)) != srs_success) {
...
srs_error_t SrsGoApiRtcPlay::do_serve_http(ISrsHttpResponseWriter* w, ISrsHttpMessage* r, SrsJsonObject* res)
{
    ...
    return do_serve_rtc_request(w, r, res, server_, is_publish);
}
