...
- `bash skills/srs-develop/scripts/srs-reload-test.sh` — config reload of a running SRS, for changes to reload, the RAW API or signal handling: reloads through `rpc=reload` and SIGHUP with a live RTMP publisher, a parse failure, a `check_config` failure and a `-e` server, each checked through `rpc=reload-fetch` and `rpc=raw`. Inotify auto-reload is Linux-only and not covered.
