...
- `scripts/oryx-stack-start.sh` — checks prerequisites, stops the existing local stack, verifies its ports are clear, then starts fresh local SRS, Go backend, and React dashboard processes. Starts Redis only if unreachable; never stops Redis. Records launch PIDs in `/tmp/oryx-stack-state.env` (override with `$ORYX_STACK_STATE_FILE`) for diagnostics, including partial startup failures.
- `scripts/oryx-stack-stop.sh` — discovers processes through the local stack's TCP listening sockets and UDP sockets, regardless of missing or stale PID state. Sends TERM, waits, escalates to KILL if necessary, and fails if ports remain occupied. Redis is never stopped. TCP clients connected to the servers are not selected.

These scripts reserve TCP ports `3000`, `2022`, `2024`, `2443`, `1935`, `1985`, `8080` and UDP ports `8000`, `10080` for the local development stack. Cleanup stops server processes occupying these ports even if started manually; do not run unrelated servers on them. Requires `lsof` and sufficient permissions to inspect and signal the services. Clean startup supports only the default local endpoints, not remote/custom endpoint overrides. Run start/stop sequentially, not concurrently, and do not restart while tests are running. Stop external supervisors first if they automatically restart these services.

Run `oryx-stack-start.sh`, then run any number of the test scripts below. None of them start or stop the shared stack themselves, so they are safe to run concurrently against it:
...
- `scripts/oryx-transcode-test.sh` — Publishes a source, enables global transcoding, verifies output playback, then explicitly disables transcoding and waits for the task to stop. Exit/signal cleanup also disables it; the previous configuration is not restored, even if previously enabled. Requires `jq` for JSON validation. Use only a development target and do not run multiple copies or change transcoding from the dashboard during the test.
...
- `scripts/oryx-ci-test.sh` — Runs the Oryx repository's own tests, the ones the GitHub Actions workflows call, limited to the cases that leave no trace on the local environment: the root `make test` set (`platform` and `releases` Go tests, `ui` lint and jest) plus the read-only black-box cases from `oryx/test/`. Pass `--unit-only` for the half that needs no stack, `--blackbox-only` for the half that does. The script header carries the allowlist of safe black-box cases and the reason each excluded one is CI-only, such as `TestApi_SetupWebsiteFooter`, which sets the ICP footer with no restore. It also works around two local quirks: `TestMain` resolves ffmpeg through a hardcoded `PATH` that omits `~/.local/bin`, and `prepareTest` loads a possibly stale `oryx/test/.env` ahead of the live credentials. `oryx-run-tests.sh` runs this one sequentially rather than in the parallel batch, because `TestSystem_LoginByPassword` has no retry and the backend's login handler uses `TryLock`.
