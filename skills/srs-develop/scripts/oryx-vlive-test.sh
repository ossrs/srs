#!/bin/bash
# E2E test for the Oryx "Virtual Live" scenario page (tab=vlive,
# ScenarioVLive.js): bind a live source stream as a vLive input, configure a
# vLive platform to forward it back into this same Oryx/SRS instance, and
# verify the forwarded copy actually shows up and plays.
#
# Source choice, worked out from oryx/platform/virtual-live-stream.go: vLive
# supports four source types (upload, server, ytdl, stream). A locally
# published RTMP stream ("stream" type, via /vlive/stream-url) is the most
# straightforward -- it needs no file upload/copy step and no youtube-dl --
# so this script publishes its own source stream over RTMP and binds that
# URL directly, mirroring TestScenario_WithStream_PublishVliveStreamUrl in
# oryx/test/scenario_test.go.
#
# Major API sequence exercised (all under /terraform/v1/ffmpeg/vlive/):
#   1. stream-url  -- validate the source RTMP URL, get back {name,uuid,target}
#   2. source      -- ffprobe the source, enforce codec/bitrate limits, bind
#                     it to a platform (persists VLiveConfigure.Files in Redis)
...
#                     VLiveConfigure.Update() *replaces* Files wholesale with
#                     whatever the update request body carries -- posting only
#                     server/secret/enabled without files would silently wipe
#                     the binding from step 2. Fetch-mutate-repost, not a
#                     fresh object.
#   4. streams     -- (diagnostics only) lists running vLive tasks with live
#                     ffmpeg status.
#
# Like oryx/platform/forward.go, there is no delete action for vlive/secret
# (only "update"), and an unset platform filter still runs whatever files are
# bound -- so a timestamped platform key here would accumulate a permanent,
# growing "Inactive" entry in Redis and the dashboard's Live Status list on
# every run (see the "Fix Oryx forward-test Redis growth" commit for the same
# issue in oryx-forward-test.sh). This script reuses one fixed platform key
# across runs and disables it when done instead.
#
...
# "vlive-" to pass the backend's allowed-platform check in virtual-live-stream.go.
PLATFORM="vlive-99-oryx-test"
...
check_srs_stream_published() {
  ...
  echo "vLive task status:" >&2
  curl -sS -m 3 -X POST "$ENDPOINT/terraform/v1/ffmpeg/vlive/streams" \
...
if ! command -v jq &>/dev/null; then
  echo "FAIL: jq not found in PATH (needed to fetch-mutate-repost the vlive config)." >&2
  exit 1
fi
...
# --- Step 2: Publish the vLive source stream via RTMP ---
echo "=== Step 1: Publish source stream via RTMP ==="
SOURCE_STREAM="vlivetest-src-$(date +%s)"
SOURCE_URL="$SRS_RTMP/$APP/$SOURCE_STREAM?secret=$PUBLISH_SECRET"
ffmpeg -re -stream_loop -1 -i "$SOURCE_FLV" -c copy -f flv \
  "$SOURCE_URL" >/tmp/oryx-vlive-source.log 2>&1 &
FFMPEG_PID=$!
sleep 3
if ! kill -0 "$FFMPEG_PID" 2>/dev/null; then
  echo "FAIL: source publisher exited early. Logs:" >&2
  cat /tmp/oryx-vlive-source.log >&2
  exit 1
fi
check_srs_stream_published "$SOURCE_STREAM" 15
echo ""

# --- Step 3: Bind the source URL via vlive/stream-url ---
echo "=== Step 2: POST /terraform/v1/ffmpeg/vlive/stream-url ==="
STREAM_URL_RESP=$(curl -sS -m 10 -X POST "$ENDPOINT/terraform/v1/ffmpeg/vlive/stream-url" \
...
# --- Step 4: Probe and bind the source to the platform via vlive/source ---
echo "=== Step 3: POST /terraform/v1/ffmpeg/vlive/source (ffprobe + bind) ==="
...
SOURCE_RESP=$(curl -sS -m 20 -X POST "$ENDPOINT/terraform/v1/ffmpeg/vlive/source" \
  -H "Authorization: Bearer $BEARER" -H 'Content-Type: application/json' \
...
echo "=== Step 4: GET+POST /terraform/v1/ffmpeg/vlive/secret (enable forward to self) ==="
CONF_RESP=$(curl -sS -m 5 -X POST "$ENDPOINT/terraform/v1/ffmpeg/vlive/secret" \
  -H "Authorization: Bearer $BEARER" -H 'Content-Type: application/json' -d '{}')
...
OUT_STREAM="vlivetest-dst-$(date +%s)"
UPDATED_CONF=$(echo "$PLATFORM_CONF" | jq -c \
  --arg server "$SRS_RTMP/$APP/" \
  --arg secret "$OUT_STREAM?secret=$PUBLISH_SECRET" \
  --arg label "oryx-vlive-test" \
  '.server=$server | .secret=$secret | .enabled=true | .custom=true | .label=$label | .action="update"')
curl -sS -m 5 -X POST "$ENDPOINT/terraform/v1/ffmpeg/vlive/secret" \
  -H "Authorization: Bearer $BEARER" -H 'Content-Type: application/json' \
  -d "$UPDATED_CONF" >/dev/null
echo "vLive config set: $PLATFORM -> $SRS_RTMP/$APP/$OUT_STREAM (forwarding back to this same Oryx/SRS)"
echo ""
...
# --- Step 7: Disable the vlive config and stop the source publisher ---
echo "=== Step 6: Disable vlive config ==="
DISABLED_CONF=$(echo "$UPDATED_CONF" | jq -c '.enabled=false | .action="update"')
curl -sS -m 5 -X POST "$ENDPOINT/terraform/v1/ffmpeg/vlive/secret" \
  -H "Authorization: Bearer $BEARER" -H 'Content-Type: application/json' \
  -d "$DISABLED_CONF" >/dev/null 2>&1 || true
echo "PASS: vlive config disabled (no delete API; this is the product's own lifecycle)."
...
echo "=== Oryx Virtual Live Test PASSED ==="
